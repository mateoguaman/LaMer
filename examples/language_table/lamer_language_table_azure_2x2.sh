...
# Azure 2x2: 2 training VMs x 2 H100s each = 4 total training GPUs.
# Env servers run on a separate VM.
#
# Required env vars (set by start_training.sh or export manually):
#   TRAIN_DATA_PATH, VAL_DATA_PATH
#   ENV_VM_IP (private IP of the env server VM)
#   TRAIN_PORT, VAL_PORT
...
env_address="${ENV_VM_IP:?'Set ENV_VM_IP'}:${TRAIN_PORT:-50051}"
val_address="${ENV_VM_IP}:${VAL_PORT:-50052}"
...
python3 -m verl.trainer.main_ppo \
    ...
    2>&1 | tee -a ${RUN_LOG_PATH:-language_table_azure_2x2.log}
