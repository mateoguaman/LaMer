#!/usr/bin/env python3
"""
Standalone RoboLab rollout using a remote vLLM server (OpenAI-compatible API).

Reuses RobolabEnvironmentManager for prompt construction and action projection,
but calls an external vLLM server instead of running an LLM locally. Single-attempt
only (no MetaRL reflection).

Usage:
    python examples/robolab/api_rollout.py \
...
def parse_args():
    ...
    p.add_argument("--remote_address", required=True,
                   help="HOST:PORT of the RoboLab env server")
    p.add_argument("--vllm_url", default="http://127.0.0.1:8000/v1",
    ...
    p.add_argument("--image_dir", type=str, default=None,
                   help="Directory to save per-step observation images (PNG)")
...
def build_env(remote_address):
    ...
    from agent_system.environments.robolab.env_manager import RobolabEnvironmentManager
    config = OmegaConf.create({"env": {"reflection_type": "history_only"}})
    remote = RemoteEnvironmentManager(remote_address, timeout=300.0)
    return RobolabEnvironmentManager(remote, config)
...
def save_obs_image(image, path):
    """Save a single numpy observation image to disk."""
...
def run_batch(env, client, model, max_turns, temperature, max_tokens,
              video_dir, batch_idx, no_think=True, human=False, image_dir=None):
    ...
    if image_dir is not None:
        init_images = obs.get("image")
        if init_images is not None:
            for i in range(num_envs):
                if init_images[i] is not None:
                    save_obs_image(
                        init_images[i],
                        os.path.join(image_dir, f"batch{batch_idx:04d}_env{i}_step_init.png"),
                    )
    ...
    for step_idx in range(effective_max):
        ...
        if active[0]:
            print(f"  --- prompt env 0 step {step_idx} ---\n{prompts[0]}\n  --- end prompt ---")
        ...
        # Save latest per-env observation images
        step_images = obs.get("image")
        if image_dir is not None and step_images is not None:
            for i in range(num_envs):
                if step_images[i] is not None:
                    save_obs_image(
                        step_images[i],
                        os.path.join(image_dir, f"batch{batch_idx:04d}_env{i}_step{step_idx:03d}.png"),
                    )

        # Build grid frames from per-env images if available
        if step_images is not None:
            valid_frames = [
                step_images[i] for i in range(num_envs)
                if step_images[i] is not None
            ]
            if valid_frames:
                grid = make_grid_frame(
                    [step_images[i] if step_images[i] is not None
                     else np.zeros_like(valid_frames[0])
                     for i in range(num_envs)],
                    step_idx,
                )
                all_episode_grid_frames.append(grid)
                if video_dir:
                    vid_path = os.path.join(
                        video_dir, f"batch{batch_idx:04d}_step{step_idx:03d}_grid.mp4"
                    )
                    save_video([grid], vid_path, fps=1)
                    print(f"  step frame -> {vid_path}")
    ...
    if video_dir and all_episode_grid_frames:
        ...
        save_video(all_episode_grid_frames, vid_path, fps=2)
    ...
    try:
        for batch_idx in range(n_batches):
            batch_results = run_batch(
                ...
                human=args.human, image_dir=args.image_dir,
            )
