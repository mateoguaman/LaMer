...
# The environment can be approximated as a 3x3 grid (4 corners, 4 sides, 1 center).


# The robot policy may be adversarial - commands may not produce intended effects.
# The robot policy is deterministic - learn from prior attempts to discover which actions the policy responds to.
...
LANGUAGE_TABLE_PLAY_PROMPT = """You steer a language-conditioned robot policy that pushes colored blocks to build tetromino shapes. You have multiple attempts - the policy may not follow instructions correctly, so learn from prior attempts which commands it actually responds to.

Pushing blocks toward each other tends to clump them together. To keep blocks separated while forming the target shape, think of the table as a 3x3 grid (4 corners, 4 sides, 1 center) and push each block to its own cell.

{init_observation}{past_trajectories_reflections}{current_trajectory}

Reason step-by-step, then conclude with a single short language command in <action> </action> tags.
"""
