"""
Modular action-disturbance system for MazeEnv.
...
override ``reset(rng)`` to draw fresh randomness at the start of each episode.
    ...
    from maze_env.disturbances import ActionDisturbance

    class MyDisturbance(ActionDisturbance):
        def __call__(self, action: int) -> int:
            return (action + 1) % 4

Pass a list of disturbances to ``MazeEnv``:

    env = MazeEnv(n=10, disturbances=[FlipLeftRight(), RotateActions()])
...
Action encoding (matches MazeEnv)
-----------------------------------
0 = UP
1 = DOWN
2 = LEFT
...
class ActionDisturbance(ABC):
    ...
    def reset(self, rng: np.random.Generator) -> None:
        """
        Called by ``MazeEnv.reset()`` before each new episode.
...
class RotateActions(ActionDisturbance):
    ...
    def reset(self, rng: np.random.Generator) -> None:
        self._k = int(rng.integers(1, _N_ACTIONS))  # k ∈ {1, 2, 3}
...
class RandomChoice(ActionDisturbance):
    ...
    Because ``MazeWorker.restart()`` restores the environment from a saved
    copy rather than calling ``reset()`` again, the chosen disturbance is
    automatically preserved across all attempts within a meta-episode.

    Include ``NoDisturbance`` in the pool to give the agent a chance of
    encountering an undisturbed maze.
    ...
    Example
    -------
        env = MazeEnv(n=8, disturbances=[
            RandomChoice([FlipLeftRight(), FlipUpDown(), NoDisturbance()])
        ])
    """
...
class Probabilistic(ActionDisturbance):
    ...
    -------
        env = MazeEnv(n=8, disturbances=[
            Probabilistic(FlipLeftRight(), p=0.5),
            Probabilistic(RotateActions(),  p=0.5),
        ])
    """
